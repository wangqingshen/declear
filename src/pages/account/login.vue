        ...
        <mu-form :model="form" class="mu-demo-form" label-width="100">
          ...
          <mu-form-item class="input-verify">
            <mu-text-field v-model="form.verify_code" ref="verifyCode" @change="inputChange(2)" placeholder="请输入短信验证码" max-length="6" underline-color="#002C5A" color="#ccc">
              <label>验证码</label>
              <VerifyCode :mob="form.mobile" :type="1" @emitSend="verifySend"/>
            </mu-text-field>
          </mu-form-item>
        </mu-form>
        <div class="btn-wrap">
          <mu-button color="#002C5A" class="login" @click.native="login" v-loading="loginLoading" data-mu-loading-size="26" data-mu-loading-color="#002C5A">登录</mu-button>
        </div>
    ...
    login(){
      ...
      this.$api.login({
          ...
          if(res.code == 1) {
            ...
            localStorage.setItem('member_state', res.data.member_state)
          }else{
            this.errorState = true
